"""Dutch-law research agent, served as a Bindu agent.

This example shows how to take the Dutch-Law-MCP server you already
have running in this repository and expose it as a fully networked,
DID-identified agent on the **Bindu** framework.

Bindu turns an in-process agent into:

  • a JSON-RPC 2.0 endpoint that speaks the A2A (agent-to-agent) protocol
    on ``http://localhost:3773``,
...
uses Agno's ``MCPTools`` to bridge the Dutch-Law-MCP tools into the
model's tool-call loop. The MCP server is spawned over stdio via
``npx``, so no local clone or build is required to run this example.
...
# ---------------------------------------------------------------------------
# 1. Spawn the Dutch-Law-MCP server over stdio
# ---------------------------------------------------------------------------
# This example assumes you have built the MCP server from source at the
# root of this repository (see the README, "Self-host this MCP"):
#
#     npm install
#     npm run build
#
# Bindu then spawns ``node dist/index.js`` once and reuses the
# connection. The MCP server's ``ensureDatabase()`` utility takes care
# of the SQLite corpus on first launch — either by using the
# repository's ``data/database.db`` if present, or by downloading the
# pre-built database from GitHub Releases into
# ``~/.cache/dutch-law-mcp/``.
# ---------------------------------------------------------------------------
...
def _clear_stale_lock(entry) -> None:
    """Clear the WASM-SQLite directory lock from any previous run.

    ``@ansvar/mcp-sqlite`` uses a ``<db>.lock/`` directory for mutual
    exclusion. The stdio entry point (``dist/index.js``) does not
    invoke ``prepareRuntimeDatabase``, so if a previous MCP process
    was killed abruptly the lock directory persists and the next
    read fails with "database is locked". This helper deletes it
    before each spawn — equivalent to ``runtime-db.ts``'s
    ``removeStaleLockDir``.
    """
    import shutil
    from pathlib import Path

    candidates = [entry.parent.parent / "data" / "database.db"]
...
def build_server_params() -> StdioServerParameters:
    ...
    raw = os.environ.get("DUTCH_LAW_MCP_ENTRY", DEFAULT_MCP_ENTRY)
    entry = Path(raw)
    if not entry.is_absolute():
        entry = (Path(__file__).parent / entry).resolve()
    if not entry.exists():
        raise FileNotFoundError(
            f"Dutch-Law-MCP entry not found at {entry}. From the repository "
            f"root, run: npm install && npm run build"
        )
    _clear_stale_lock(entry)
    env = {"PATH": os.environ.get("PATH", "")}
    if db := os.environ.get("DUTCH_LAW_DB_PATH"):
        env["DUTCH_LAW_DB_PATH"] = db
...
# explicitly, or let the agent pick: if only ``OPENROUTER_API_KEY`` is
# present we route through OpenRouter, otherwise we fall back to
# OpenAI. Any OpenRouter model slug works (e.g. ``openai/gpt-4o``,
...
# re-open the 130 MB database. Instead we open the connection lazily
# on the first request, behind an ``asyncio.Lock``, and reuse it from
# every subsequent request. This is the supported Agno pattern for
...
config: dict[str, Any] = {
    ...
    "description": (
        "Citation-grounded Dutch legal research, served on Bindu. Answers "
        "questions in Dutch or English against 3,251 Dutch statutes (BWB) "
        "with verbatim provision text from wetten.overheid.nl, via the "
        "Ansvar dutch-legal-citations MCP server. Research only — not "
        "legal advice."
    ),
    "version": "0.1.0",
    "deployment": {
