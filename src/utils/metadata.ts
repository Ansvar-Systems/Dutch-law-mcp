...
export function generateResponseMetadata(db?: Database): ResponseMetadata {
  ...
  return {
    data_freshness: dataFreshness,
    disclaimer:
      'NOT LEGAL ADVICE. This tool is for research purposes only. Always verify citations with official sources (wetten.overheid.nl, rechtspraak.nl) before relying on them in legal matters.',
    source_authority: {
      primary_source:
        'wetten.overheid.nl (official BWB) for statutes; rechtspraak.nl (official open data) for case law',
      authority_level: 'official',
      verification_required:
        'ALWAYS cross-check with official sources before using in professional legal work.',
    },
    coverage_gaps: [
    ...
    ai_disclosure:
      'AI-assisted legal research tool. Results generated by algorithmic search and may contain errors or omissions. Human review required before professional use.',
  };
}
...
function getDataFreshness(db: Database): DataFreshness {
  ...
  try {
    const row = db
      .prepare(`SELECT last_sync_date FROM case_law_sync_metadata WHERE id = 1`)
  ...
  if (statuteLastUpdated) {
    ...
    if (ts < thirtyDaysAgo) {
      warnings.push(
        `Statute data is ${Math.floor((now.getTime() - ts.getTime()) / (24 * 60 * 60 * 1000))} days old`,
      );
    }
  } else {
  ...
  if (caseLawLastSync) {
    ...
    if (ts < thirtyDaysAgo) {
      warnings.push(
        `Case law data is ${Math.floor((now.getTime() - ts.getTime()) / (24 * 60 * 60 * 1000))} days old`,
      );
    }
  }
